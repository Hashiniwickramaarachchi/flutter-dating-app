...
class A_OnlineStatusService with WidgetsBindingObserver {
  ...
  Future<void> updateUserStatus() async {
    ...
    if (user != null) {
      String sanitizedEmail =
          sanitizeEmail(user.email.toString()); // Sanitize the email

      // Mark the user online in Firestore
      await _firestore.collection('Ambassdor').doc(user.email).set(
          {
            'status': 'online',
      ...
      DatabaseReference userStatusRef =
          _databaseRef.child('status/$sanitizedEmail');
  ...
  Future<void> setUserOffline() async {
    ...
    if (user != null) {
      ...
      userStatusRef.set({
        'status': 'offline',
        'lastSeen': ServerValue
            .timestamp // Use Firebase server timestamp to ensure accurate time
      });
    }
  }
}
